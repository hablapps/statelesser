...
trait FromSemantic {
  ...
  def fromSemantic(sem: Semantic): SSelect = 
    SSelect(selToSql(sem.select), tabToSql(sem.table), whrToSql(sem.where))
  ...
  private def tabToSql(tab: Map[Var, Tree]): SqlFrom = 
    SFrom(List(tab.toList match {
      case (Var(nme), GLabel(info)) :: xs => 
        STable(info.tgt.nme, nme, xs.map(joinToSql))
      case _ => throw new Error(s"No table was selected for FROM clause")
    }))

  private def joinToSql(vt: (OpticLang.Var, Tree)): SqlJoin = vt match {
    case (Var(n1), VL(Var(n2), GLabel(inf))) => SEqJoin(
      s"${inf.tgt.nme}", n1, SOn(SProj(n2, inf.nme), SProj(n1, "id")))
